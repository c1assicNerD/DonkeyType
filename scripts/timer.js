document.fonts.ready.then(() => {
  // Your code to measure and set border height
    ...
    console.log((textchild.firstChild.firstChild.getBoundingClientRect().height*3))
...
choices.addEventListener('click',async function(e){
    ...
    switch (temp.children[1].textContent){
        ...
        case "customize":
            ...
            break;
        default:
        console.log("Why was this(default) triggered in the first place(maybe something has gone horribly wrong)")
    }
...
for(let i =1;i<=counter;i++)  xValues.push(i)
   ...
   //     clear = setTimeout(interval,1000)
   // })();
...
async function QuoteOption(e){
    if(!e){
        ...
        //anyWithoutEvent =  document.querySelector('.button').firstElementChild;
        //anyWithoutEvent.classList.add("modeSelected");
        e ={
        target:{
...
function customGame(){
    switch(customizeChoice.mode){
        ...
        case "custom":
            ...
            console.log("line 348, ",game_words)
            maxWords = game_words.length;
